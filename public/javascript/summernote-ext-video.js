  ...
  var createVideoNode = function (url) {
    ...
    if (ytMatch && ytMatch[1].length === 11) {
      ...
      $video = $('<video controls>')
        .attr('src', url)
        .attr('width', '640').attr('height', '360');
    } else if (url.indexOf("http://v.youku.com/v_show/id")!=-1) {
      var vedioObj = url.substring(url.indexOf("id_")+3,url.indexOf(".html"));
      $video = $('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>')
        //.attr('height', '498')
        //.attr('width', '510')
        //.attr('src', '//player.youku.com/embed/' + vedioObj);   //youkuMatch[1]);
          .attr('src', 'http://player.youku.com/embed/' + vedioObj );   //youkuMatch[1]);
      //.attr('src', sUrl);   //youkuMatch[1]);
    }else {
