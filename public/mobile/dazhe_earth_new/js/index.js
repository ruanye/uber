        ...
        success:function(data){
            if(data.status==-1){
                ...
                if(data.status==1){
                    ...
                    //新用户
                    setTimeout(function(){$("#page2").hide();$("#page6").show();},500);
                }
            }
        },
        error:function(error){
            console.log(JSON.stringify(error));
            $("#convert_btn").html("<div>领取失败</div>");
            setTimeout(function(){$("#page2").hide();$("#page6").show();},1000);
        }
    });
}
